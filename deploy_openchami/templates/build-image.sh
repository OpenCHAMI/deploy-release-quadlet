...
function build-image() {
    local config="${1}"; shift || {
        _bi_fail "build-image: image config file not specified"
        return 1
    }
    ...
    [[ -f "${config}" ]] || {
        _bi_fail "build-image: config file '${config}' not found"
        return 1
    }
    podman run \
           --network=host \
           ...
           --config config.yaml \
           --log-level DEBUG || {
        _bi_fail "build-image: can't build image found in '${config}'"
        return 1
    }
}
...
function build-image-rh9() {
    local config="${1}"; shift || {
        _bi_fail "build-image-rh9: image config file not specified"
        return 1
    }
    build-image "${config}"
}
...
function build-image-rh8() {
    local config="${1}"; shift || {
        _bi_fail "build-image-rh8: image config file not specified"
        return 1
    }
...
function generate-boot-config() {
    local image_subpath="${1}"; shift || {
        _bi_fail "generate-boot-config: image subpath (example 'compute/debug') not provided as first argument"
        return 1
    }
    local headnode_ip="${1}"; shift || {
        _bi_fail "generate-boot-config: management head-node IP address not provided as second argument"
        return 1
    }
    local macs="$(for mac in "$@"; do echo "${mac}"; done)"
    local s3_port="{{ openchami_config.s3.api_port }}"
    [[ "${macs}" != "" ]] || {
        _bi_fail "generate-boot-config: no target node MAC addresses provided"
        return 1
    }
    cd /opt/workdir/boot
    ...
    [[ "${uri_img}" != "" ]] || {
        _bi_fail "generate-boot-config: no disk image found that matches '${image_subpath}'"
        return 1
    }
    local uri_initramfs="$(echo "${uris}" | cut -d' ' -f2)"
    [[ "${uri_initramfs}" != "" ]] || {
        _bi_fail "gnerate-boot-config: no initrd image found that matches '${image_subpath}'"
        return 1
    }
    local uri_kernel="$(echo "${uris}" | cut -d' ' -f3)"
    [[ "${uri_kernel}" != "" ]] || {
        _bi_fail "generate-boot-config: no kernel image found that matches '${image_subpath}'"
        return 1
    }
...
generate-boot-config-json() {
    generate-boot-config "$@" | yaml_to_json | jq "."
}

function __bmc_user() {
    local name="${1}"; shift || {
        _bi_fail "BMC name not provided for __bmc_user"
        return 1
    }
...
function __bmc_password() {
    local name="${1}"; shift || {
        _bi_fail "BMC name not provided for __bmc_password"
        return 1
    }
    sudo cat /etc/vtds/bmc_info.json | \
...
function __node_reset() {
    local reset_type="${1}"; shift || {
        _bi_fail "no reset type supplied"
        return 1
    }
    local node_name="${1}"; shift || {
        _bi_fail "no node name provided"
        return 1
    }
    local bmc_name="${1}"; shift || {
        _bi_fail "no BMC name provided"
        return 1
    }
    local bmc_url="https://${bmc_name}/redfish/v1/Systems"
    local node_action="${node_name}/Actions/ComputerSystem.Reset"

    # After DNS is switched, it can take a little time for coredns to
    # be populated. Give it a bit of time to resolve...
    retries=10
    while true; do
        if curl -k \
                 ...
                 "${bmc_url}/${node_action}"; then
            break
        fi
        [[ "$((retries--))" -le 0 ]] && {
            _bi_fail "timed out trying to '${reset_type}' node '${node_name}' at '${bmc_url}/${node_action}'"
            return 1
        }
        sleep 10
    done
